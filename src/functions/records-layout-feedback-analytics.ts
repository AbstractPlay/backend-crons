...
import {
    ...
    pruneProcessedKeys,
    utcDateKey,
} from "../utils/layoutFeedbackAnalytics.js";

const REGION = "us-east-1";
const OPS_BUCKET = "private-ops-153672715141-us-east-1-an";
const ANALYTICS_PREFIX = "gamemove-layout/analytics";
...
async function scanLayoutFeedbackEvents(tableName: string, minSk: string): Promise<RawDdbItem[]> {
...
export const handler: Handler = async () => {
    ...
    console.log(`Scanning ${tableName} for ${LAYOUT_FB_PK_PREFIX}* with sk >= ${minSk}`);

    const rawItems = await scanLayoutFeedbackEvents(tableName, minSk);
    console.log(`Scanned ${rawItems.length} layout feedback event rows`);
    ...
    console.log(
        `Wrote ${SUMMARY_KEY}: sessions=${summaryBody.totals.sessions}, `
        + `notes=${summaryBody.totals.notesSubmitted}, `
        + `bailouts=${summaryBody.totals.classicBailouts}`,
    );
